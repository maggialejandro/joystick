define(["backbone", "models/ship", "vector2"], function(Backbone, ShipModel, Vector2) {
    ...
    var ShipView = Backbone.View.extend({
      ...
      update: function(){
        ...
        if(this.vel.isMagGreaterThan(0)) this.model.set({angle: this.vel.angle()});
      ...
      draw: function(){
        ...
        if(this.model.get('thrustSize')>0) {
          ...
          var styleStr = "translate3d("+posx+"px, "+posy+"px, 0px) rotate("+this.get('angle')+"deg)";
          this.canvas.style.webkitTransform = this.canvas.style.MozTransform = this.canvas.style.OTransform = this.canvas.style.transform = styleStr;
